...
dv.table = function(input)
{
    ...
    table["names"] = [] // holds column names
...
outer:            
    ...
    table.toJSON = function() {
        ...
        json = []
        console.log(table)
        table[0].forEach(function(value, index) {
            item = {}
            names.forEach(function(n) {
                item[n] = table[n].get(index)
            })
            json.push(item)
        })
        return json
    ...
    table.extend_to = function(other_table) {
        other_table["names"] = table["names"]
        other_table.query = table.query
        other_table.sparse_query = table.sparse_query
        other_table.where = table.where
        other_table.toJSON = table.toJSON
        return other_table
    }
    return table;
};
